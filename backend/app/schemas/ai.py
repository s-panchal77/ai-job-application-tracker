...
class AIMatchResponse(BaseModel):
    ...
    matched_skills: list[str] = Field(
        default_factory=list, description="Skills found in both resume and JD"
    )
    missing_skills: list[str] = Field(
        default_factory=list, description="Skills in the JD but missing from the resume"
    )
    suggestions: list[str] = Field(
        default_factory=list, description="Actionable improvement suggestions"
    )
    provider: str = Field(
        ..., description="Which AI provider generated this result: 'mock' or 'openai'"
    )
